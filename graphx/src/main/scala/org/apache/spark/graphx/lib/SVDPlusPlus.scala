...
object SVDPlusPlus {
    ...
    : (Graph[(DoubleMatrix, DoubleMatrix, Double, Double), Double], Double) =
  {
    ...
    : Iterator[(VertexId, Double)] =
    {
      val (usr, itm) = (et.srcAttr, et.dstAttr)
      val (p, q) = (usr._1, itm._1)
      val tmp = q.dot(usr._2)
      var pred = u + usr._3 + itm._3 + tmp
      pred = math.max(pred, conf.minVal)
      pred = math.min(pred, conf.maxVal)
      val err = (et.attr - pred) * (et.attr - pred)
      println("user: " + et.srcId + " item: " + et.dstId + " rate: " + et.attr + " predRate: " + pred +
      " bu: " + usr._3 + " bi: " + itm._3 + " pq: " + tmp)
    ...
    var preG : Graph[(DoubleMatrix, DoubleMatrix, Double, Double), Double] = null
    ...
    for (i <- 0 until conf.maxIters) {
      ...
      val t4 = testG.mapReduceTriplets(mapTestF(conf, u), (g1: Double, g2: Double) => g1 + g2)

      testG = testG.outerJoinVertices(t4) {
        (vid: VertexId, vd: (DoubleMatrix, DoubleMatrix, Double, Double), msg: Option[Double]) =>
          if (msg.isDefined) (vd._1, vd._2, vd._3, msg.get) else vd
      }
      testG.cache()

      val err = testG.vertices.map{ case (vid, vd) =>
        if (vid % 2 == 1) vd._4 else 0.0
      }.reduce(_ + _) / testG.triplets.count()
      println("iteration i: " + i + "  err is " + err)
      testG.unpersistVertices(blocking = false)
      testG.edges.unpersist(blocking = false)

    }

    // calculate error on training set
//    g.cache()
    println("Graph Overview: Vertices [%s] Edges [%s] Triplet [%s].".format(g.vertices.count(), g.edges.count(), g.triplets.count()))
